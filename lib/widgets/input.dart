...
mixin _DragSelectionMixin<T extends ConsumerStatefulWidget>
  ...
  void startDragSelection(String id, DragStartDetails details) {
    final index = dragSelectionIds.indexOf(id);
    if (index == -1) return;
    _dragSelectionValue = !dragSelectedIds.contains(id);
    _lastDragSelectionIndex = index;
    setDragSelected(id, _dragSelectionValue);
    _updateDragSelectionAt(details.globalPosition);
  }

  void updateDragSelection(DragUpdateDetails details) {
    _updateDragSelectionAt(details.globalPosition);
  }
  ...
  void _updateDragSelectionAt(Offset globalPosition) {
    ...
    if (index == null || lastIndex == null || index == lastIndex) return;
    final start = index < lastIndex ? index : lastIndex;
    final end = index > lastIndex ? index : lastIndex;
    for (var current = start; current <= end; current++) {
      setDragSelected(dragSelectionIds[current], _dragSelectionValue);
    }
    _lastDragSelectionIndex = index;
  }

  void endDragSelection([DragEndDetails? details]) {
