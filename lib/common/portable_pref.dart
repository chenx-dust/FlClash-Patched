...
bool isPortableDirectory(String appDirPath) {
  return (Platform.isWindows || Platform.isLinux) &&
      Directory(join(appDirPath, 'config')).existsSync();
}

void configurePortablePreferences({String? appDirPath}) {
  final resolvedAppDirPath = appDirPath ?? dirname(Platform.resolvedExecutable);
  if (!isPortableDirectory(resolvedAppDirPath)) {
    return;
  }
  SharedPreferencesStorePlatform.instance = PortableSharedPreferencesStore(
    File(join(resolvedAppDirPath, 'config', 'shared_preferences.json')),
  );
}
