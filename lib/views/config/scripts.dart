...
class _ScriptsViewState extends ConsumerState<ScriptsView> {
  ...
  Future<void> _handleDelete() async {
    ...
    final scripts = ref.read(scriptsProvider.notifier).value;
  ...
  Future<void> _clearEffect(int id) async {
    ...
    await _remoteUrlFutures.remove(id);
  }
  ...
  Future<void> _handleSyncScript(Script script) async {
    ...
    try {
      ...
      ref
          .read(updatingKeysProvider.notifier)
          .stop(updatingKey, updatingOperation);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final selectedScript = selectedScripts.length == 1
        ...
        : null;
    final isSelectedScriptUpdating = selectedScripts.any(
      (script) => ref.watch(isUpdatingProvider(script.updatingKey)),
    );
            ...
            CommonMinIconButtonTheme(
              child: IconButton.filledTonal(
                ...
                onPressed: isSelectedScriptUpdating
                    ? null
                    : () {
                        unawaited(_handleSyncScript(selectedScript));
                      },
          ...
          if (selectedScriptIds.isNotEmpty) ...[
            CommonMinIconButtonTheme(
              child: IconButton.filledTonal(
                ...
                onPressed: isSelectedScriptUpdating ? null : _handleDelete,
