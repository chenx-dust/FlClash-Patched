...
final class SharedStateStore {
  ...
  func loadTunnelConfiguration() -> TunnelConfiguration {
    guard let userDefaults = UserDefaults(suiteName: appGroupIdentifier),
          ...
          let sharedState = try? JSONDecoder().decode(SharedStatePayload.self, from: data) else {
      return TunnelConfiguration()
    }
    return TunnelConfiguration(
      options: sharedState.vpnOptions?.networkExtensionOptions ?? NetworkExtensionOptions(),
  ...
  func saveRunTime() {
    ...
    UserDefaults(suiteName: appGroupIdentifier)?
      .set(milliseconds, forKey: runTimeKey)
  }

  func clearRunTime() {
    UserDefaults(suiteName: appGroupIdentifier)?
      .removeObject(forKey: runTimeKey)
  }
}
